...
pub trait SourceCheckoutExt {
    ...
    ) -> impl std::future::Future<Output = Result<SourceCheckout, SourceCheckoutError>> + Send;
...
impl SourceCheckoutExt for ComputeCtx {
    async fn pin_and_checkout(
        &mut self,
        source_location_spec: SourceLocationSpec,
    ) -> Result<SourceCheckout, SourceCheckoutError> {
        match source_location_spec {
            ...
            SourceLocationSpec::Path(path) => Ok(SourceCheckout {
                path: self.resolve_typed_path(path.path.to_path())?,
    ...
    async fn checkout_pinned_source(
        &mut self,
        pinned_spec: PinnedSourceSpec,
    ) -> Result<SourceCheckout, SourceCheckoutError> {
        match pinned_spec {
            PinnedSourceSpec::Path(ref path_spec) => {
                let source_path = self.resolve_typed_path(path_spec.path.to_path())?;
                Ok(SourceCheckout {
                    path: source_path,
                    pinned: pinned_spec,
                })
