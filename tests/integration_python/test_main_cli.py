...
def test_remove_finds_packages_across_tables(pixi: Path, tmp_pixi_workspace: Path) -> None:
    """A bare `pixi remove` locates each package wherever it is defined and
    removes it, without the user having to pass `--pypi`/`--feature`/etc."""
...
"""
    ...
    assert "numpy" not in parsed.get("dependencies", {})
    # An untouched dependency in the same table is kept.
    assert "ruff" in parsed.get("dependencies", {})
    assert "black" not in parsed.get("pypi-dependencies", {})
    assert "only-here" not in (
        parsed.get("target", {}).get(CURRENT_PLATFORM, {}).get("dependencies", {})
    )
    assert "pytest" not in parsed.get("feature", {}).get("dev", {}).get("dependencies", {})


def test_remove_ambiguous_location_errors(pixi: Path, tmp_pixi_workspace: Path) -> None:
    """When the same package lives in more than one table a bare `pixi remove`
    refuses to guess and points at the disambiguating commands."""
    manifest = tmp_pixi_workspace / "pixi.toml"
    manifest.write_text(
        EMPTY_BOILERPLATE_PROJECT
        + """
...
"""
    ...
    verify_cli_command(
        ...
        ExitCode.FAILURE,
        stderr_contains=["multiple locations", "--feature dev"],
    )

    # Nothing was removed.
    parsed = tomllib.loads(manifest.read_text())
    assert "numpy" in parsed["dependencies"]
    assert "numpy" in parsed["feature"]["dev"]["dependencies"]


def test_remove_missing_package_suggests_similar(pixi: Path, tmp_pixi_workspace: Path) -> None:
    """A bare `pixi remove` of an absent package reports a workspace-wide miss
    and suggests a similarly-named package."""
    manifest = tmp_pixi_workspace / "pixi.toml"
    manifest.write_text(
        EMPTY_BOILERPLATE_PROJECT
        + """
...
"""
    ...
    verify_cli_command(
        ...
        ExitCode.FAILURE,
        stderr_contains=["was not found in the workspace", "numpy"],
    )
